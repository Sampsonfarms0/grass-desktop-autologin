...
def send_xdotool_key(key_sequence: str, retry_multiplier: int, delay_ms: str = XDOTOOL_TYPE_DELAY_MS) -> bool:
    ...
    # Keys that should use 'key' instead of 'type'
    special_keys = ["Tab", "Return", "Enter", "BackSpace", "Escape"]

    # Decide whether to type or keypress
    if key_sequence in special_keys:
        cmd = XDOTOOL_CMD + ["key", key_sequence]
    else:
        # ALWAYS use type for strings (fixes @ . ! etc)
